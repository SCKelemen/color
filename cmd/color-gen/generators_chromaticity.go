...
func generateChromaticity() error {
	...
	for py := 0; py < img.Bounds().Dy(); py++ {
		for px := 0; px < img.Bounds().Dx(); px++ {
			if img.RGBAAt(px, py).A == 0 {
				...
				nearest := findNearestColorChromaticity(img, px, py, 10)
				if nearest.A > 0 {
...
func findNearestColorChromaticity(img *image.RGBA, x, y, radius int) color.RGBA {
	for r := 1; r <= radius; r++ {
		for dy := -r; dy <= r; dy++ {
			for dx := -r; dx <= r; dx++ {
				if dx*dx+dy*dy > r*r {
...
func drawSpectralLocus(img *image.RGBA, offsetX, offsetY, scaleX, scaleY, minX, minY, maxY float64) {
	...
	drawLineChromaticity(img, int(offsetX+scaleX*(xyCoords[0].x-minX)), int(offsetY+scaleY*(maxY-xyCoords[0].y)),
	...
	for i := 0; i < len(xyCoords)-1; i++ {
		...
		drawLineChromaticity(img, x1, y1, x2, y2, color.RGBA{150, 150, 150, 255}, 2)
	}
}
...
func drawGamutBoundary(img *image.RGBA, offsetX, offsetY, scaleX, scaleY, minX, minY, maxY float64, createColor func(r, g, b float64) col.Color) {
	...
	if len(points) >= 3 {
		...
		// Draw triangle edges
		drawLineChromaticity(img, x1, y1, x2, y2, color.RGBA{255, 255, 255, 255}, 2)
		drawLineChromaticity(img, x2, y2, x3, y3, color.RGBA{255, 255, 255, 255}, 2)
		drawLineChromaticity(img, x3, y3, x1, y1, color.RGBA{255, 255, 255, 255}, 2)
	}
}
...
func drawLineChromaticity(img *image.RGBA, x0, y0, x1, y1 int, col color.Color, thickness int) {
	dx := abs(x1 - x0)
	dy := abs(y1 - y0)
	sx := 1
	if x0 > x1 {
