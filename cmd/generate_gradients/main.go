...
const (
	...
	labelHeight    = 30
	cornerRadius   = 8
	padding        = 8  // Reduced from 20 to bring labels closer
...
func init() {
	// Try to load Roboto font from fonts/Roboto directory
	robotoFonts := []string{
	...
	// Final fallback: use basicfont (will be scaled)
	defaultFace = nil
	defaultTT = nil
}
...
func main() {
	...
	for _, sp := range spaces {
		...
		for _, textColor := range []string{"black", "white"} {
			...
			finalImg := scaleDown(img, gradientWidth, gradientHeight+padding+20)
...
func drawLabels(img *image.RGBA, centerY int, start, end col.Color, spaceName, textColor string, scale int) {
	...
	if shadowColor != "" {
		drawTextScaled(img, padding*scale+scale, textY+scale, startStr, shadowColor, false, scale) // shadow first
	}
	drawTextScaled(img, padding*scale, textY, startStr, primaryColor, false, scale) // primary text on top

	// Center: space name (uppercase)
	if shadowColor != "" {
		drawTextScaled(img, (gradientWidth*scale)/2+scale, textY+scale, strings.ToUpper(spaceName), shadowColor, true, scale) // shadow first
	}
	drawTextScaled(img, (gradientWidth*scale)/2, textY, strings.ToUpper(spaceName), primaryColor, true, scale) // primary text on top
	...
	if shadowColor != "" {
		drawTextScaled(img, rightX+scale, textY+scale, endStr, shadowColor, false, scale) // shadow first
	}
	drawTextScaled(img, rightX, textY, endStr, primaryColor, false, scale) // primary text on top
}
